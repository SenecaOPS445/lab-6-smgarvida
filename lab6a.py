...
class Student:
    ...
    def __init__(self, name, number):
        self.name = name
        self.number = str(number)  # Convert number to string to avoid TypeError in displayStudent
        self.courses = {}
    ...
    def displayGPA(self):
        if not self.courses:
            return 'GPA of student ' + self.name + ' is 0.0'  # Handle division by zero if no courses
...
if __name__ == '__main__':
    ...
    student2 = Student('Jessica', 123456)  # Using integer for student number
